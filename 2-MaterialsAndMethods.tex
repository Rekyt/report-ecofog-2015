...
%We used those models~\eqref{eq:delta} to understand how the intra-specific variability of single trait influenced AGR, we generalized them to all the traits taken together to understand the interplay between traits:
%
%\begin{equation}
%	\label{eq:multi}
%	\delta = \left\{
%	\begin{aligned}
%	\sum_{k = 1}^{K} \prescript{4}{}\theta_k \text{Tr}_{k,s} & \\
%	\sum_{k = 1}^{K} \prescript{4}{}\theta_k \text{Tr}_{k,s} & + \prescript{5}{}\theta_k \times(\text{Tr}_{k,i} - \text{Tr}_‘k,s}) \\
%	\sum_{k = 1}^{K} \prescript{4}{}\theta_k \text{Tr}_{k,s} & + \prescript{5}{}\theta_k \times \vert \text{Tr}_{k,i} - \text{Tr}_{k,s} \vert \\
%	\sum_{k = 1}^{K} \prescript{5}{}\theta_k \text{Tr}_{k,i} \\
%	\end{aligned}
%	\right.,
%\end{equation}
%
%with $K$ the total number of traits, and the other terms described as above depending on the given trait $k$.
